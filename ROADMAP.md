...
**Правила:** нумерация сквозная (1…35); внутри цепочки — строго по порядку; один пункт = один прогон самоулучшения; после проверки — `complete_self_improvement_item`.
...
**12 · S · Prompt caching Claude/облако** — приор. High  
- **Цель:** system + tools кэшируются через `cache_control: { type: 'ephemeral' }`; −90% стоимости кэшированных токенов и ниже латентность на каждом шаге ReAct  
- **Файлы:** `app/electron/main/providers/claudeProvider.ts`, при необходимости `openaiProvider.ts`, `geminiProvider.ts`  
- **Действие:** обернуть системный промпт и массив ~70 tools в cacheable content blocks (`@anthropic-ai/sdk` ^0.105.0)  
- **Проверка:** на 2-й итерации ReAct в ответе API `cache_read_input_tokens` > 0; `npm run typecheck`

**13 · M · Чекпоинт прогона и «Откатить всё»** — приор. High  
...
**14 · M · Маскирование секретов в логах и контексте** — приор. High  
...
**15 · M · @-упоминание файлов в поле ввода** — приор. High  
...
**16 · S · Nudge «используй RAG»** — приор. Medium  
...
**17 · L · Символьный индекс (find_symbol)** — приор. High  
...
**18 · M · Дерево файлов проекта** — приор. High  
...
**19 · M · Side-by-side diff** — приор. Medium  
...
**20 · S · Уведомление «агент закончил»** — приор. Medium  
...
**21 · M · Шаблоны чатов** — приор. Medium  
...
**22 · M · Авто-PR collective** — приор. Medium  
...
**23 · M · Рейтинг знаний collective** — приор. Low  
...
**24 · S · Экспорт урока в skill** — приор. Medium  
...
**25 · M · Контракт subagent** — приор. Medium  
...
**26 · L · Explorer subagent** — приор. Medium  
...
**27 · L · Editor subagent в цикле** — приор. Low  
...
**28 · S · Каналы обновлений stable/beta** — приор. Low  
...
**29 · L · Песочница для run_script** — приор. Medium  
...
**30 · L · Голосовой ввод и озвучка** — приор. Low  
...
**31 · XL · LSP в редакторе** — приор. Low  
...
**32 · L · Skill marketplace** — приор. Low  
...
**33 · M · E2E на Linux/macOS в CI** — приор. Medium  
...
**34 · L · Авто-цикл «тесты → почини»** — приор. Medium  
...
**35 · M · Счётчик стоимости облачных запросов** — приор. Medium  
