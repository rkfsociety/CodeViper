...
|---|---|---|---|---|---|
| **Производительность** | В `SettingsModal.tsx` добавить готовые профили суммаризации: «Экономичный» (55%), «Сбалансированный» (70%), «Качество» (85%); кнопки-пресеты над слайдером, клик выставляет значение | Medium | S | — | ⏳ |
| **Производительность** | В `services.ts` добавить LRU-кэш 500 записей для результатов `grep_files`; ключ `{query, path, glob}`, инвалидация при изменении любого файла в директории `path` | Medium | M | — | ⏳ |
| **Производительность** | В `fileSearchInWorker.ts` улучшить батчинг: группировать до 5 одновременных grep/find запросов в один worker-вызов, сокращая повторный обход файловой системы | Medium | M | — | ⏳ |
...
| **UI/UX** | В `OllamaDownloadStatus.tsx` добавить расчёт средней скорости загрузки за последние 10 с и вывод «Осталось ~N мин» | Low | S | — | ⏳ |
| **UI/UX** | В `ChatHistoryPanel.tsx` добавить группировку чатов по `projectPath` (как папки); автоматически разворачивать группу активного чата | Low | M | — | ⏳ |
| **UI/UX** | Создать `FileTimelinePanel.tsx`: показывает NDJSON-лог изменений файла с временной шкалой; вызывается из контекстного меню файла в ответе агента | Low | L | — | ⏳ |
