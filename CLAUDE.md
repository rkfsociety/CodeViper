...
- **Если задача была из ROADMAP — после редактирования кода сразу обновить `ROADMAP.md`**: перенести пункт из «В планах» в «✅ Сделано», перенумеровать оставшиеся пункты. Делать это до коммита, одним коммитом вместе с кодом. Не ждать, пока пользователь спросит — это обязательная часть выполнения задачи из ROADMAP.
...
- **Если задача требует изменений в 3+ файлах или затрагивает несколько модулей — не начинать реализацию сразу.** Сначала декомпозировать: разбить на конкретные подзадачи (каждая — одна атомарная единица работы), добавить в `ROADMAP.md` в раздел «В планах» в подходящую группу, **затем сообщить пользователю список подзадач и ждать его команды «делай».**
