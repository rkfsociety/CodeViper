...
- Порядок перед каждым коммитом с кодом: `npm run typecheck` → `npm run build` → **обновить ROADMAP и README** (перенести выполненные задачи; проверить витрину проекта) → `git commit` → `git push`. Версию — **только** в релизном коммите (см. release-workflow).
