...
- Если нужный инструмент (node, npm, npx, git, python и т.д.) отсутствует в среде — скачать в `C:\github\Prog_II` и установить оттуда.
- **Никогда не пропускать typecheck/тесты из-за отсутствия инструмента** — сначала установить, потом выполнить.
- Порядок: `Get-Command node -ErrorAction SilentlyContinue` → если нет, скачать MSI с nodejs.org в `C:\github\Prog_II\`, `msiexec /qn`, обновить `$env:PATH` из реестра → продолжить.
