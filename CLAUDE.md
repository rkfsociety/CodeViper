...
- **После любых правок — всегда `git commit` в том же сеансе**, не копить незакоммиченное: `git add` → `git commit`. Для изменений в `app/` — сначала `npm run build`, потом коммит. **`git push` — только по явной просьбе**; без push runtime на `.exe` обновится только после push + `git pull` в source.

## Версионирование

- **Версию в `app/package.json` менять только при выпуске релиза** — не после каждого коммита и не после каждой задачи. Порядок: `.cursor/rules/release-workflow.mdc` (CI зелёный → bump → push → тег `vX.Y.Z`).
- **Схема версий — semver:** `MAJOR.MINOR.PATCH`
  - `PATCH` — багфикс, мелкое улучшение
  - `MINOR` — новая фича, новый инструмент/провайдер
  - `MAJOR` — кардинальное изменение архитектуры или публичный релиз
- Поднять версию: только поле `"version"` в `app/package.json`. `package-lock.json` не трогать вручную.
- **Не bump'ить версию** при обычных коммитах (код, docs, ROADMAP, самоулучшение без релиза).
...
- Порядок перед каждым коммитом с кодом: `npm run typecheck` → `npm run build` → **обновить ROADMAP и README** → `git commit`. Версию — **только** в релизном коммите (см. release-workflow).
- **`git push` — только по запросу пользователя.** Runtime на машине пользователя обновляется из GitHub (`git pull` в `%APPDATA%\CodeViper\source`); после локального коммита без push напомнить про необходимость push, когда пользователь будет готов.
- **Не откладывать коммит «на потом»** после завершённой задачи. Незакоммиченные правки теряются при сбросе; push можно отложить до явной просьбы.

## После каждой выполненной задачи (ROADMAP)

- **Если задача была из ROADMAP — после редактирования кода сразу обновить `ROADMAP.md`**: удалить пункт из «В планах», перенумеровать с 1, добавить запись в `ROADMAP_DONE.md`. Одним коммитом с кодом.
- **После любого коммита с изменением кода — проверить `ROADMAP.md`**: найти задачи, которые реализованы этим изменением, и перенести их в `ROADMAP_DONE.md`. Не ждать отдельной просьбы — это часть порядка перед коммитом.
...
- **Сначала убедиться, что задача ещё не реализована.** Перед тем как писать код — прочитать целевой файл и при необходимости сделать `git log --oneline -20`. Если функциональность уже есть в коде — задача выполнена: перенести пункт в `ROADMAP_DONE.md` и сообщить пользователю, не писать код повторно.
...
Агент: читает пункт → `set_self_improvement_plan` по Действие/Проверка → код → проверка → пункт в `ROADMAP_DONE.md`, правка `ROADMAP.md` в том же коммите.
...
**Добавить запись в `ROADMAP_DONE.md`** — в раздел `## Сделано` вставить строку вида:
```
...
**Порядок перед коммитом:** `typecheck` → `build` → обновить ROADMAP/README → `git commit` (push — по запросу).
