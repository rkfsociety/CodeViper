...
| `anti-repeat.mdc` | Обязательно читать rules/README, проверять memories и записывать проверенные решения |
...
2026-06-30 · user preference — не предлагать пользователю опциональные «следующим шагом могу...» или похожие развилки, если можно просто сразу исправить/починить проблему. При наличии исправления действовать без запроса подтверждения и без лишних развилок.

2026-06-30 · hotkeys App.tsx — все горячие клавиши сосредоточены в едином `keydown`-обработчике в `app/src/App.tsx`; для разбора часто смотреть ветку `Ctrl+P`, `Ctrl+,`, `Ctrl+K`, `Ctrl+Shift+N`, `Ctrl+Shift+T`, `Ctrl+B`, `Ctrl+\`` и `?` вместе с `KeyboardShortcutsModal`.

2026-06-30 · roadmap self-improvement — если пользователь присылает пункт из ROADMAP.md, это означает задачу на реализацию в агенте CodeViper, а не только на текстовый отчёт; сначала проверить, существует ли нужный tool/handler, и если нет — добавить его в код.

2026-06-30 · roadmap encoding safety — длинные PowerShell-команды с кириллицей могут ломать текст в ROADMAP.md и ROADMAP_DONE.md; для таких правок безопаснее пересобирать файлы из чистой версии `git HEAD`, а не править их через длинные inline-строки.

2026-07-01 · roadmap rewrite safety — если `ROADMAP/*.md` или `ROADMAP_DONE.md` начинают превращаться в `?`/mojibake, не пропускать проблему и не оставлять ее как есть. Правильный путь: сразу исправить файл целиком чистым UTF-8 через один `apply_patch`, затем проверить чтение UTF-8-aware скриптом, пересчитать пункты/нумерацию и только потом коммитить.

2026-07-01 · PowerShell command separators — в этом окружении не использовать `&&` для цепочки команд: PowerShell может не принять этот разделитель. Выполнять staging/status/commit и другие шаги отдельными tool-вызовами или использовать корректный для PowerShell `;`, если последующая команда не должна зависеть от exit code предыдущей.
