...
- **2026-06-28 · trace 1782674386194** — ROADMAP п.1 custom endpoint (qwen2.5-coder:7b): после разведки модель 12× писала «Вывод: Чтение … завершено» без tool calls; orchestrator считал попытки → 4 пункта заблокированы, status ok. Исправлено: `looksLikeFakeToolOutput` + passthrough в orchestrator, `FAKE_TOOL_OUTPUT_NUDGE`, error при 0 done и все blocked, `stripLeadingOrphanBrace` для `}\n{"name":…}`.
