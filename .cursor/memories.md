...
- **2026-06-28 · trace 1782666530617** — ROADMAP п.1 (qwen2.5-coder:7b): не читал ROADMAP (угадывал README/agent.ts); `edit_codeviper_file ../README.md` — «вне проекта» (read работал, edit — нет); PR без коммитов; бесконечный цикл при 2 заблокированных пунктах (`hasPending` без actionable). Исправлено: `codeViperReadRoot` в `assertCodeViperPath`, `hasActionablePending` → done, `resolvePlanToolArg` action/check, проверка коммитов перед PR, nudge `read_roadmap_item`. — ROADMAP п.1 (custom endpoint, qwen2.5-coder:7b): 10+ вызовов `edit_codeviper_file` с `content`/`new_content`/`file_path` вместо `old_string`/`new_string` → `undefined.trim()` / abort. Исправлено: `resolveEditToolArgs`, подсказка `EDIT_WRONG_ARGS_HINT`, `read_codeviper_file` через `resolveToolPathArg`, пути `app/` в ROADMAP п.1.
