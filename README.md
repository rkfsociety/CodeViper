...
- [📋 Дорожная карта](ROADMAP.md) — 28 задач в формате для самообучения (цель / файлы / действие / проверка); промпт: `Выполни пункт N из ROADMAP.md — самоулучшение CodeViper`
...
P2P «Поделиться мощностью»: настройки → «Интеграции»; при первом включении — диалог согласия (что передаётся: URL Ollama и модель; лимиты нагрузки CPU/GPU). Без «Принимаю» тумблер остаётся выключенным; флаг `p2pConsentGiven` сохраняется в настройках. При CPU > 15% или GPU > 20% входящие P2P-задачи ставятся на паузу; не более 3 параллельно — остальные ждут в очереди до 60 с, затем 503 (`p2pClient.ts`).
