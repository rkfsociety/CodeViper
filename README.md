...
Локальный AI-агент с GUI: читает проект, дерево файлов, side-by-side diff, стеклянная панель «Недавние» с pill-списком чатов, иконка в трее, toast при завершении агента, экспорт трейса агента в JSON (папка данных приложения, не репозиторий), отправка трейса в GitHub Issue через `gh` (кнопка «На GitHub» в панели «Трасса», нужен `gh auth login`), панели «Метрики» и «Трасса» с перетаскиваемым разделителем, ищет символы по AST (`find_symbol`), правит файлы, защита от «вечной разведки» и чтения вне списка «Файлы:» в ROADMAP, распознавание копипаста пункта ROADMAP как самоулучшения, запускает команды, git stash/pop и откат, самоулучшение. Ollama, DeepSeek, OpenAI, Anthropic, Gemini, OpenRouter.
...
| [API (TypeDoc)](https://rkfsociety.github.io/CodeViper/) | [ROADMAP](ROADMAP.md) (135 задач) · [выполнено](ROADMAP_DONE.md) |
