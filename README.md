...
Работает **полностью локально** через [Ollama](https://ollama.com) или подключается к **облачным API** — DeepSeek, OpenAI, OpenRouter (400+ моделей). Исходный код открыт: агент может читать и улучшать сам себя.
...
| ☁️ **Облачные модели** | DeepSeek API, OpenAI-совместимые, OpenRouter (GPT-4o, Claude, Gemini…) |
| 🛠 **Инструменты агента** | Чтение/запись/поиск файлов, grep, терминал, git-операции, предпросмотр правок |
...
|---|---|---|
| Ollama | не нужен | Локально, любые GGUF-модели |
...
│   │   ├── providers/     # Ollama, OpenAI, DeepSeek, OpenRouter
