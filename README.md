...
Локальный AI-агент с GUI: читает и правит код, запускает команды и git, самоулучшение по ROADMAP. Провайдеры — Ollama, OpenAI, custom endpoint (LM Studio, vLLM), Anthropic, Gemini, DeepSeek, OpenRouter; запасные модели при 429/5xx. В чате по умолчанию скрыт поток reasoning think-моделей (только «Думаю…»); включить текст — Настройки → Поведение. Возможности и сценарии — в [вики](https://github.com/rkfsociety/CodeViper/wiki).
