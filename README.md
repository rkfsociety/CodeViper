...
- [📋 Дорожная карта](ROADMAP.md) — 27 задач в формате для самообучения (цель / файлы / действие / проверка); промпт: `Выполни пункт N из ROADMAP.md — самоулучшение CodeViper`
...
P2P «Поделиться мощностью»: настройки → «Интеграции»; при первом включении — диалог согласия (что передаётся: URL Ollama и модель; лимиты нагрузки CPU/GPU). Без «Принимаю» тумблер остаётся выключенным. Промпты между узлами шифруются ECDH (X25519) + AES-256-GCM; сигнальный сервер (`server/p2p/`) ретранслирует только ciphertext по HTTPS/WSS — plaintext в логах сервера не пишется. TLS на сервере: `TLS_KEY_PATH` и `TLS_CERT_PATH` в `.env`. Лимиты: CPU/GPU пауза, до 3 параллельных задач, очередь 60 с.
