...
- [📋 Дорожная карта](ROADMAP.md) — 26 задач в формате для самообучения (цель / файлы / действие / проверка); промпт: `Выполни пункт N из ROADMAP.md — самоулучшение CodeViper`
...
P2P «Поделиться мощностью»: настройки → «Интеграции»; при первом включении — диалог согласия. Промпты шифруются ECDH + AES-256-GCM; сигнальный сервер (`server/p2p/`) ретранслирует ciphertext по HTTPS/WSS. Маршрутизация: `POST /tasks/route` ищет свободный онлайн-узел с нужной моделью, иначе `{ fallback: true }`. TLS: `TLS_KEY_PATH` / `TLS_CERT_PATH`. Лимиты: CPU/GPU пауза, до 3 параллельных задач.
