...
Локальный AI-агент с GUI: читает проект, дерево файлов, side-by-side diff, иконка в трее, toast при завершении агента, экспорт трейса агента в JSON (папка данных приложения, не репозиторий), панели «Метрики» и «Трасса» с перетаскиваемым разделителем, ищет символы по AST (`find_symbol`), правит файлы, защита от «вечной разведки» и чтения вне списка «Файлы:» в ROADMAP, распознавание копипаста пункта ROADMAP как самоулучшения, запускает команды, git stash/pop и откат, самоулучшение. Ollama, DeepSeek, OpenAI, Anthropic, Gemini, OpenRouter.
