...
Установщик клонирует репозиторий в **`%APPDATA%/CodeViper/source`** (нужен **Git for Windows**). При **автообновлении** без полной переустановки: синхронизация с `origin/master` (сброс ветки `agent/*` и локальных правок в клоне) → сборка в `source/app` → баннер **«Перезапустить для применения»** — подтягиваются agent runtime и **интерфейс** (окно, настройки, селектор моделей) из клона. Тонкая оболочка `CodeViper.exe` меняется редко; новый установщик — только при смене Electron/NSIS или первой установке.
...
| [API (TypeDoc)](https://rkfsociety.github.io/CodeViper/) | [ROADMAP](ROADMAP.md) (118 задач) · [выполнено](ROADMAP_DONE.md) |
