...
export function shouldRetryForMissingTools(
  ...
  // («Давайте изучу…») и остановилась. Повтор нужен только если ответ короткий.
  // Если ответ длинный (>200 символов) — скорее всего облачная модель (DeepSeek, GPT)
  // ответила из своих знаний; принимаем как есть, иначе уходим в бесконечный retry.
  if (!mutationTask && !anyToolsUsed && assistantText.trim().length < 200) return true
