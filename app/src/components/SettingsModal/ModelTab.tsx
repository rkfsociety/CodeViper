          ...
          <div className={styles.section}>
            ...
            <label className={styles.toggle}>
              <input
                ...
                onChange={(e) =>
                  onSettingsChange({ updateChannel: e.target.checked ? 'beta' : 'stable' })
                }
              />
              Beta-версии (pre-release)
            </label>
            <div style={{ fontSize: 11, opacity: 0.55, marginTop: 4 }}>
              Включить — обновляться до pre-release (нестабильные сборки). По умолчанию только
              stable-релизы. Вступает в силу при следующем запуске.
            </div>
          </div>
