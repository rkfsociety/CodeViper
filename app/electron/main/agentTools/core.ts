...
export const FILE_TOOLS = [
  ...
  },
  {
    ...
    function: {
      ...
      description:
        'Проверяет P2P credits: отрицательный баланс, NaN и лимиты в server/p2p/src/credits.ts; при доступности читает runtime balance из live store.',
      parameters: {
        type: 'object',
        properties: {
          path: {
  ...
  },
  {
    type: 'function',
    function: {
      name: 'find_p2p_connection_issues',
      description:
        'Report invalid WSS URLs, reconnect backoff, timeout and health-check for p2pClient.ts',
      parameters: {
        type: 'object',
        properties: {
          path: {
            type: 'string',
            description: 'Path to p2pClient.ts or app/electron/main (optional)'
          }
        }
      }
    }
