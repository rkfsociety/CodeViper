...
Установщик (packaged): `electron-updater` + GitHub Releases, баннер «Перезапустить и обновить». Разработка из исходников: git fetch `app/` и перезапуск через лаунчер.
