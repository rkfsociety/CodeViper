...
`remember`, `search_memory`, `forget`, `list_skills`, `read_skill`, `create_skill`, `update_skill`, `delete_skill`, `read_skill_data`, `write_skill_data`, `find_skill_file_issues`, `find_index_param_issues`, `set_todo_list`, `complete_todo_item`, `clear_todo_list`, `check_cve`, `web_fetch`, `web_search
...
Поиск маркеров незавершённого merge-конфликта (`<<<<<<<`, `=======`, `>>>>>>>`) по файлам проекта. Возвращает отчёт `[n] path:line` или «не найдено». Параметр: `path` (необяз., подпапка).
...
Отчёт о некорректных параметрах индексации: chunk size, overlap и batch в `settings.ts` и RAG-файлах (`rag.ts`, `contextRAG.ts`, `embeddingQueue.ts`). Возвращает читаемый Markdown-отчёт в чат.
...
Проверка Docker Compose на конфликты опубликованных портов: дубли host-port и публикации без host bind. Параметр `path` (необяз., файл `docker-compose*.yml` или подпапка для поиска).

### `find_docker_env_issues`

Проверка `docker-compose` `environment`/`env_file`: переменные, которых нет в `.env.example`, и потенциально неполные примеры окружения. Параметр `path` (необяз., файл compose или подпапка).
